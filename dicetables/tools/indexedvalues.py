...
class IndexedValues(object):
    ...
    def combine_with_dictionary(self, no_zero_values_dict):
        ...
        new_raw_values = [sum(elements) for elements in zip(*container_for_lists_to_combine)]
...
def generate_indexed_values_np(sorted_tuple_list):
    """
...
def generate_indexed_values_np_from_dict(input_dict):
    """

    :param input_dict: may not be empty.\n
        {int: int>0, ...}
    """
    return generate_indexed_values_np(sorted(input_dict.items()))


def make_start_index_and_list_np(sorted_tuple_list):
    start_val = sorted_tuple_list[0][0]
    end_val = sorted_tuple_list[-1][0]
    out_list = [0] * (end_val - start_val + 1)
    for index, value in sorted_tuple_list:
...
class IndexedValuesNp(object):
    ...
    def combine_with_dictionary(self, no_zero_values_dict):
        ...
        for event, occurrences in no_zero_values_dict.items():
            index_offset = event - first_event
            new_group_of_events = get_events_list_np(base_list, occurrences, new_size, index_offset)
            container_for_lists_to_combine.append(new_group_of_events)
        new_raw_values = [sum(element) for element in zip(*container_for_lists_to_combine)]
        return IndexedValuesNp(new_start_index, new_raw_values)


def get_events_list_np(base_list, occurrences, new_size, index_offset):
    if occurrences != 1:
        adjusted_events = [value * occurrences for value in base_list]
    else:
        adjusted_events = base_list[:]
    return change_list_len_with_zeroes_np(adjusted_events, new_size, index_offset)


def change_list_len_with_zeroes_np(input_list, new_size, index_offset):
    left = [0] * index_offset
    right = [0] * (new_size - index_offset - len(input_list))
    return left + input_list + right
